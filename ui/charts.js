...
export function axes(W,H,ageStart,ageEnd,maxBal,{ layout, fmtM, grid, axisInk }){
  ...
  for(let i=0;i<=4;i++){const v=maxBal*(1-i/4); const y=y0+(y1-y0)/4*i;
    g+=`<text x="${x0-12}" y="${y+4}" fill="${axisInk}" font-size="11" style="font-family:var(--font-sans)" text-anchor="end">${fmtM(v)}</text>`;}
  // x age ticks
  const span=ageEnd-ageStart;
  for(let k=0;k<=5;k++){const a=Math.round(ageStart+span*k/5); const x=x0+(x1-x0)*k/5;
    g+=`<text x="${x}" y="${H-9}" fill="${axisInk}" font-size="11" style="font-family:var(--font-sans)" text-anchor="${k===0?'start':k===5?'end':'middle'}">Age ${a}</text>`;}
  return g;
}
...
export function storyChart(rows,{ layout, fmtM }){
  ...
  for(const f of [0,.5,1]){
    ...
    g += `<text x="${padL-8}" y="${gy+4}" fill="var(--ink-faint)" font-size="10.5" style="font-family:var(--font-sans)" text-anchor="end">${fmtM(hi*(1-f))}</text>`;
  }
  ...
  for(let a = Math.ceil(a0/5)*5; a <= a1; a += 5){
    ...
    g += `<text x="${ax}" y="${H-8}" fill="var(--ink-faint)" font-size="10.5" style="font-family:var(--font-sans)" text-anchor="middle">${a===Math.ceil(a0/5)*5?'Age '+a:a}</text>`;
  }
