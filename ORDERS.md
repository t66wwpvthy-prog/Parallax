...
- **Rejected — do NOT resurrect:** the green restyle, the dark "Midnight Analyst" navy
  theme, the light "paper" theme, the light **"porcelain glass" theme** (shipped
  2026-06-07, rejected by Nathan the same night — work preserved on
  `claude/porcelain-glass`), the "two retirees same returns opposite order" idea,
...
- 2026-06-07 · General · canonical+main · **REVERTED the porcelain glass retheme.** Nathan reviewed it live and rejected it ("i dont like it"); reset `claude/dev` + `main` back to the copper build (`4fa305f`). The full porcelain + 8px-grid work is PRESERVED on branch `claude/porcelain-glass` (`7310563`) in case of a later revisit — do NOT resurrect the light theme unless Nathan explicitly asks (added to the Rejected list above). Live theme stays dark copper-glass. Engine 42/42 (copper build unchanged).
- 2026-06-07 · General · canonical+main `9a10b72` · (a) Cash flow drawer now full-width (not capped to scenario columns), max-height 72vh for better visibility on client screens. Added cfShowAll toggle ("All scenarios" / "Compare two") so all scenario columns display at once; useful for wide-screen comparisons. (b) Sequencing y-axis fix: ceiling = max(entry×1.5, realMax×1.15) — starting balance now ~30% of height (room for growth), all path endings visible with headroom. Refined niceCeil ladder (added steps 10/12/15) to avoid overshooting from 25M → 30M. Both changes address Nathan feedback "starting port value at the upper end...paths exit the graph...need longer term stable incline." Verify 10/10, engine 42/42.
- 2026-06-07 · General · canonical+main `f253931` · Session batch: (a) Scenarios — editable per-column goal amounts + Reset-to-baseline button, dropped Buy Property lever; (b) engine cash-flow path fix — two-stage medoid (top-10% central by balance THEN closest-to-median return-volatility) so the "representative" path has average volatility, not a lucky-recovery-from-3-down-years sequence (engine 42/42); (c) Household header redesign (Variant A two-tier People/Children/Plan — names roomy, no stranded buttons); (d) Goals tab → "Living Expenses & Goals"; (e) scaled up Net Worth ledgers (split page 50/50) + Scenarios category labels (gutter 184→218, bigger one-line labels) — Nathan-approved; (f) Medicare + out-of-pocket PRELOAD as a 2nd foundation card ($6,500/person, age 65→plan end), stored as age-gated expenses.extra so the engine consumes it with no new math (verified −5.6pts at $13k); LTC stays baked in engine. NOT building Suggestion-4 rankable essentials. Font readability pass deferred to the color redesign (Nathan's call). Verify 10/10 throughout.
